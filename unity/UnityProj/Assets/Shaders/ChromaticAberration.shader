    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(v2f i) : SV_Target {
                ...
                // Apply subtle distortion curve - cubic falloff for gradual effect
                float distortionFactor = _Distortion * distance * distance * distance;
                
                // Calculate aberration strength based on distance from center
                float aberrationAmount = _Aberration * distortionFactor;
                
                // Only apply effect if we're reasonably far from center (reduces center artifacts)
                if (distance < 0.2) {
                ...
                // Moderate channel displacement - visible but not overwhelming
                // Red channel: moderate shift outward
                float2 redUV = uv + direction * aberrationAmount * 0.5;
                
                // Green channel: slight shift for more realistic look
                float2 greenUV = uv + direction * aberrationAmount * 0.1;
                
                // Blue channel: moderate shift inward
                float2 blueUV = uv - direction * aberrationAmount * 0.4;
